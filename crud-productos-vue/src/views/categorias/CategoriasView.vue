...
const filtered = computed(() =>
  categorias.value.filter((item) => {
    const term = search.value.trim().toLowerCase()
    const matchesText =
      !term || item.codigo.toLowerCase().includes(term) || item.nombre.toLowerCase().includes(term)
...
function errorMessage(error) {
  return error.response?.data?.message || error.response?.data?.detail || t('categories.loadError')
}
...
async function load() {
  ...
  serviceError.value = ''
  try {
    categorias.value = await listarCategorias(locale.value)
  } catch (error) {
...
async function edit(item) {
  ...
  try {
    const [es, en] = await Promise.all([
      obtenerCategoria(item.id, 'es'),
      obtenerCategoria(item.id, 'en'),
    ])
    editing.value = {
      id: item.id,
      codigo: es.codigo ?? en.codigo ?? item.codigo,
...
async function save(data) {
  ...
  serviceError.value = ''
  try {
    if (editing.value) await actualizarCategoria(editing.value.id, data, locale.value)
    else await crearCategoria(data, locale.value)
    const wasEditing = Boolean(editing.value)
    formOpen.value = false
    await load()
    notice.value = t(wasEditing ? 'categories.updated' : 'categories.created')
...
async function toggleStatus() {
  ...
  try {
    ...
    await cambiarEstadoCategoria(selected.value.id)
    confirmOpen.value = false
    await load()
    notice.value = t(activating ? 'categories.activated' : 'categories.deactivated')
    setTimeout(() => (notice.value = ''), 3000)
  } catch (error) {
    serviceError.value = errorMessage(error)
  } finally {
...
<template>
  <PageHeader
    ...
    ><template #actions
      ><button class="btn btn-primary" @click="create">
        <i class="bi bi-plus-lg me-2"></i>{{ t('categories.new') }}
      </button></template
    ></PageHeader
  >
    ...
    <div class="card-body p-0">
      <div class="filters row g-3 p-3 p-lg-4">
        <div class="col-md-7 col-lg-5">
          <SearchInput v-model="search" :placeholder="t('categories.search')" />
        </div>
      ...
      <LoadingSpinner v-if="cargando" :text="t('categories.loading')" />
        ...
        <table class="table table-hover align-middle mb-0">
          ...
          <tbody>
            <tr v-for="item in filtered" :key="item.id">
              ...
              <td>
                <strong class="category-name">{{ item.nombre }}</strong
            ...
            <tr v-if="!filtered.length">
              <td colspan="4" class="empty-state text-center">
                <i class="bi bi-search d-block"></i>{{ t('categories.empty') }}
              </td>
            </tr>
          </tbody>
        </table>
      ...
      >
        <small class="text-secondary">{{
          t('common.records', { shown: filtered.length, total: categorias.length })
        }}</small>
  ...
  <BaseModal
    ...
    <CategoriaForm :categoria="editing" :saving="guardando" @submit="save" /><template #footer
      ><button class="btn btn-light" :disabled="guardando" @click="formOpen = false">
        {{ t('common.cancel') }}</button
      ><button type="submit" form="categoria-form" class="btn btn-primary" :disabled="guardando">
        <span v-if="guardando" class="spinner-border spinner-border-sm me-2"></span
        >{{ editing ? t('common.saveChanges') : t('categories.create') }}
      </button></template
    ></BaseModal
  >
  <ConfirmModal
    ...
    :message="
      selected
        ? t(selected.activo ? 'categories.confirmDeactivate' : 'categories.confirmActivate', {
